...
app.whenReady().then(() => {
  ...
  ipcMain.on('ollama:stream-prompt', async (event, { prompt, model, contextFiles, systemPrompt, settings }) => {
    ...
    try {
      ...
      You are Lumina 2.0.
      PROTOCOL:
      1. Answer immediately. Do NOT use <thinking> tags.
      2. Be concise and technical.
      3. Use standard Markdown for code.
